...
var form = formModal.querySelector('div form');
...
var areasInput = document.getElementById('interest_area-checklist');
var estatesInput = document.getElementById('estate_type-checklist');
var countryInput = form.querySelector('input[name="nationality"]');
var countryList = document.getElementById('country-list');

var countryListItems = _toConsumableArray(countryList.children); //TEXT INPUT (name and job)


var verifyText = function verifyText(el) {
  var reviseTip = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  if (el.value.length > 3) el.classList.add('verified');else el.classList.remove('verified');
  if (reviseTip && el.classList.contains('verified')) el.nextElementSibling.style.display = 'none';
};

nameInput.addEventListener('input', function (e) {
  return verifyText(e.target, true);
});
jobInput.addEventListener('input', function (e) {
  return verifyText(e.target);
});
...
emailInput.addEventListener('focus', function () {
  return emailInput.nextElementSibling.style.display = 'none';
});
...
var verifyCountry = function verifyCountry() {
  //verify country
...
countryInput.addEventListener('focus', function (e) {
...
var countryListValues = countryListItems.map(function (e) {
  return e.innerText;
});
countryInput.addEventListener('input', function (e) {
  countryListItems.forEach(function (item) {
    if (!item.innerText.includes(countryInput.value)) item.style.display = 'none';else item.style.display = 'list-item';
  });
  verifyCountry();
}); //fill input and verify on country click

countryListItems.forEach(function (li) {
  li.addEventListener(clickEvent, function (e) {
...
var verifyPhone = function verifyPhone(el) {
  var reviseTip = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  if (/^\+?\d{9,15}$/.test(el.value)) el.classList.add('verified');else el.classList.remove('verified');
  if (!reviseTip) return;
  ...
  if (el.classList.contains('verified')) el.nextElementSibling.style.display = 'none';else el.nextElementSibling.style.display = 'block';
};

phoneInput.addEventListener('input', function (e) {
  return verifyPhone(e.target);
});
whatsappInput.addEventListener('input', function (e) {
  return verifyPhone(e.target);
});
phoneInput.addEventListener('focusout', function (e) {
  return verifyPhone(e.target, true);
});
whatsappInput.addEventListener('focusout', function (e) {
  return verifyPhone(e.target, true);
}); //CHECKBOX LISTS

var verifyCheckboxList = function verifyCheckboxList(x) {
  var count = _toConsumableArray(x.nextElementSibling.nextElementSibling.querySelectorAll('input')).filter(function (i) {
    return i.checked;
  }).length;

  var currentCount = x.firstElementChild.innerHTML;
...
areasInput.addEventListener('input', function (e) {
  return verifyCheckboxList(e.target);
});
estatesInput.addEventListener('input', function (e) {
  return verifyCheckboxList(e.target);
});

var expandCheckboxes = function expandCheckboxes(x) {
  x.classList.toggle('expand-checkbox');
  if (x.classList.contains('expand-checkbox')) return;
  verifyCheckboxList(x);
};

areasInput.addEventListener(clickEvent, function (e) {
  return expandCheckboxes(e.target);
});
estatesInput.addEventListener(clickEvent, function (e) {
  return expandCheckboxes(e.target);
}); //VERIFY ALL INPUTS

var verifyAllInputs = function verifyAllInputs() {
  verifyEmail();
  ...
  var verifiedCount = formModal.querySelectorAll('.verified').length;
...
formModal.addEventListener('click', function (e) {
  ...
  _toConsumableArray(formModal.querySelectorAll('.expand-checkbox')).forEach(function (i) {
    ...
    if (!i.classList.contains('expand-checkbox')) {
      verifyCheckboxList(i); //checklist tip if check-list closed but not verified
...
var checkFormStatus = function checkFormStatus() {
  if (formModal.querySelectorAll('.verified').length == 8) formModal.querySelector('[type="submit"]').disabled = false;else formModal.querySelector('[type="submit"]').disabled = true;
};
...
var visitorRegister = function visitorRegister() {
...
document.getElementById('lang-btn').addEventListener('click', function () {
  ...
  if (lang == 'en') url.searchParams.set('l', 'ar');else url.searchParams.set('l', 'en');
...
window.onpopstate = function (event) {
  if (formModal.style.display == 'none') return;
  formModal.style.display = 'none'; // formModal.firstElementChild.classList.add('slide-down')
};

var expoDate = new Date("Mar 09, 2022 00:00:00:");
var countdownDiv = document.getElementById('countdown');
var now = new Date().getTime();
var timeLeft = expoDate - now; //remaining time array

var days = timeLeft / (1000 * 60 * 60 * 24);
var hours = days % 1 * 24;
var minutes = hours % 1 * 60;
var seconds = minutes % 1 * 60; //four elements: day, hour, min, sec

var counterUnits = _toConsumableArray(countdownDiv.querySelectorAll('.flex > h2'));

var timeLeftArr = [days, hours, minutes, seconds];
var i = 0;
counterUnits.forEach(function (el) {
